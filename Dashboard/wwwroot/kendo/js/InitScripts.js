...
function showSuccessMsg(str) {
    $("#popupNotification").kendoNotification({
        autoHideAfter: 700, position: { pinned: true, top: 40, left: 50, bottom: null, right: null }
    });
    $("#popupNotification").data("kendoNotification").show(str, "success");
};
function info(str) {
    $("#popupNotification").kendoNotification({
        autoHideAfter: 5000, position: { pinned: true, top: 40, left: 50, bottom: null, right: null }
    });
...
function showErrorMsg(str) {
    $("#popupNotification").kendoNotification({
        autoHideAfter: 10000, position: { pinned: true, top: 40, left: 50, bottom: null, right: null }
    });
...
function initMap(mapId) {
    ...
    if (mapContainer.data('set-marker-onclick')) {
        ...
        map.on('click', function (e) {
            ...
            $('#' + id + '_Longitude').val(e.latlng.lng + "," + e.latlng.lat);
...
var kendeHandlers = {
    ...
    gridDataSourceDelete: function (evt) {
        ...
        var Authorization = grid.dataSource.transport.options.read.headers.Authorization
        var deleteUrl = grid.element.data('delete-url');
        var dataItem = grid.dataItem($(target).closest("tr"));
            ...
            if (data.filter && data.filter.filters) {
                for (var i = 0; i < data.filter.filters.length; i++) {
                    ...
                    if (typeof (data.filter.filters[i].value) == "number") {
                        data.filter.filters[i].value = data.filter.filters[i].value.toString()
                    }
                    else {
                        ...
                        if (split.length > 2) {
                            var jdate = data.filter.filters[i].value;
                                ...
                                jD = JalaliDate.jalaliToGregorian(jdate.split('/')[0], jdate.split('/')[1], jdate.split('/')[2]);

                            data.filter.filters[i].value = jD[0] + "/" + jD[1] + "/" + jD[2]; 
    ...
    gridDataSourceError: function (e) {
        ...
        if (e.errorThrown == "UnCustomAuthorized") {
...
function initKendo(container) {
    ...
    container.find('.k-button.k-grid-createcustome').on('click', function () {
    ...
    container.find('.k-button.k-button-md.k-rounded-md.k-button-solid.k-button-solid-base.k-toolbar-button.k-toolbar-tool').on('click', function () {
        var grid = $(this).parents('[data-role=grid]');
        if (!grid.length)
            grid = $(this).parents('[data-role=treelist]');
...
$(document).ready(function () {
    ...
    $("body").on("click", "[data-submit='ajax']", function (evt) {
        ...
        if (elements == "#") {
            if (window.event.target.form == undefined) {
                elements = "#" + window.event.target.offsetParent.form.action.split('fromid=')[1]
            } else {
                elements = "#" + frm[0].action.split("fromid=")[1]
            }
        }
...
function ToDateTimePicker(e) {

    var date = Date(e.replaceAll("/", "").replaceAll("Date(", "").replaceAll(")", ""))
    var newdate = new Date(date);

    return mds.MdsPersianDateTimePicker.convertDateToString(newdate, false, 'yyyy/MM/dd');
...
function addMdDateTimePicker(container, options) {
    ...
    var persianDate = fieldValue ? fieldValue: "";
    ...
    $('#' + options.field).change(function () {
        ...
        if (selectedPersianDate) {
            ...
            var gregorianDate = selectedPersianDate;
